...
OCR.prototype = {
   ...
   * neural network.
   *
   * Load Image -> Analyze -> Extract Character(s) -> Normalize
   * -> Run through OCR -> Call callback with the results.
   */
  runOnFile: function(file, callback) {
    fs.readFile(file, function(error, source) {
      if (error) throw error;
      var img = new canvas.Image;
      img.src = source;

      var context = new canvas(img.width).getContext('2d');
    });
  },
